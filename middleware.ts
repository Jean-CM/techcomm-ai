...
function clearSupabaseCookies(request: NextRequest) {
  ...
  for (const cookie of request.cookies.getAll()) {
    if (cookie.name.startsWith("sb-") || cookie.name.includes("auth-token")) redirect.cookies.set(cookie.name, "", { path: "/", maxAge: 0 });
  }
  return redirect;
}
...
function clearSupabaseCookiesWithoutRedirect(request: NextRequest) {
  ...
  for (const cookie of request.cookies.getAll()) {
    if (cookie.name.startsWith("sb-") || cookie.name.includes("auth-token")) response.cookies.set(cookie.name, "", { path: "/", maxAge: 0 });
  }
  return response;
}
...
export async function middleware(request: NextRequest) {
  ...
  const isCrmArea = request.nextUrl.pathname.startsWith("/dashboard") || request.nextUrl.pathname.startsWith("/crm") || request.nextUrl.pathname.startsWith("/admin") || request.nextUrl.pathname.startsWith("/call-center");
...
export const config = {
  matcher: ["/dashboard/:path*", "/crm/:path*", "/admin/:path*", "/call-center/:path*", "/tecnico/:path*", "/login", "/change-password", "/api/crm/:path*"]
};
