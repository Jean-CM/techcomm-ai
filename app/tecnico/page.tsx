...
export default async function TechnicianHomePage() {
  ...
  const { data: technician } = await admin.from("technicians").select("id,full_name").eq("user_id", user.id).maybeSingle();
  ...
  const { data: orders } = await admin.from("work_orders")
  ...
  const [{ data: customers }, { data: appointments }] = await Promise.all([
    customerIds.length ? admin.from("customers").select("id,full_name,address,sector,province,municipality,address_reference_1,address_reference_2,phone").in("id", customerIds) : Promise.resolve({data:[]}),
    ...
    <main style={{ minHeight:"100vh", background:"#0b1117", color:"#edf5fa", fontFamily:"Inter,system-ui,sans-serif", padding:"18px 14px 32px" }}>
      <div style={{ maxWidth:720, margin:"0 auto" }}>
        <header style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:16,marginBottom:22}}>
          <div><div style={{fontSize:11,fontWeight:800,letterSpacing:".16em",color:"#42bde8"}}>TECHCOMM OPERATIONS · TÉCNICO</div><h1 style={{fontSize:24,margin:"5px 0 2px"}}>Mis órdenes</h1><p style={{margin:0,color:"#8197aa",fontSize:13}}>Hola, {technician.full_name.split(" ")[0]}. Solo aparecen las órdenes asignadas a ti.</p></div>
        ...
        <div style={{background:"#0f1a22",border:"1px solid #1f3442",borderRadius:13,padding:"12px 14px",marginBottom:16,color:"#a9bdca",fontSize:12,lineHeight:1.5}}>
          <strong style={{color:"#edf5fa"}}>Flujo de trabajo:</strong> abre una orden con <strong style={{color:"#42bde8"}}>Gestionar cliente</strong>, registra cuándo sales, cuándo llegas al cliente y luego indica si el servicio fue completado, requiere diagnóstico, necesita repuesto/cotización, queda pendiente por cliente o no aplica.
        </div>

        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><strong style={{fontSize:14}}>Órdenes asignadas</strong><span style={{fontSize:12,color:"#7e94a7",background:"#111a23",border:"1px solid #22313d",borderRadius:999,padding:"5px 9px"}}>{(orders ?? []).length} orden{(orders ?? []).length===1?"":"es"}</span></div>
        ...
        {(orders ?? []).length===0 ? <div style={{background:"#111a23",border:"1px solid #21303c",borderRadius:14,padding:"34px 20px",textAlign:"center"}}><div style={{fontSize:30,marginBottom:10}}>✓</div><strong>Todo al día</strong><p style={{color:"#8298aa",fontSize:13,marginBottom:0}}>No tienes órdenes pendientes por ahora.</p></div> : (orders ?? []).map(order => {
          ...
          return <section key={order.id} style={{background:"#111a23",border:"1px solid #21313e",borderRadius:14,padding:16,marginBottom:12}}>
            ...
            <p style={{margin:"0 0 10px",fontSize:13,color:"#9db0bf"}}>{order.issue || "Sin detalle de falla"}</p>
            <div style={{fontSize:12,color:"#7890a3",lineHeight:1.6,marginBottom:13}}>
              <div><strong style={{color:"#b6c7d2"}}>{customer?.full_name || "Cliente sin nombre"}</strong>{customer?.phone ? ` · ${customer.phone}` : ""}</div>
              <div>{[customer?.address,customer?.sector,customer?.municipality,customer?.province].filter(Boolean).join(", ") || "Dirección pendiente"}</div>
              {customer?.address_reference_1 && <div>Ref. 1: {customer.address_reference_1}</div>}
              {customer?.address_reference_2 && <div>Ref. 2: {customer.address_reference_2}</div>}
              {appointment?.starts_at && <div style={{marginTop:3,color:"#a8bac8"}}>Cita: {new Date(appointment.starts_at).toLocaleString("es-DO",{dateStyle:"medium",timeStyle:"short"})}</div>}
            </div>
            <a href={`/tecnico/orden/${order.id}`} style={{display:"flex",alignItems:"center",justifyContent:"center",width:"100%",boxSizing:"border-box",padding:"12px 14px",borderRadius:10,background:"#21aee0",color:"#061017",fontWeight:800,fontSize:13,textDecoration:"none"}}>Gestionar cliente</a>
          </section>;
        })}
      </div>
    </main>
