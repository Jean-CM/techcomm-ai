...
export async function POST(request: Request) {
  ...
  if (hour < 8 || hour >= 18) return NextResponse.json({ ok: false, error: "Las visitas se coordinan entre 8:00 a. m. y 6:00 p. m." }, { status: 400 });
  ...
  return NextResponse.json({ ok: true, appointment: data, reminder_scheduled_for: reminderAt });
}
...
export async function PATCH(request: Request) {
  ...
  if (body.action === "reschedule") {
    ...
    changes.status = "rescheduled"; changes.confirmation_status = "pending"; changes.starts_at = new Date(body.starts_at).toISOString();
  }
