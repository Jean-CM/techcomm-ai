...
type ManagementPayload = {
  type?: "Cliente" | "Producto" | "Cita" | "Orden" | "Cotización" | "Venta";
  name?: string;
  detail?: string;
  secondary?: string;
};
...
export async function POST(request: Request) {
  ...
  if (type === "Cita") {
    ...
    const { data, error } = await supabase.from("appointments").insert({
      ...
      notes: detail || "Servicio por definir",
    }).select().single();
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
    return NextResponse.json({ ok: true, record: data });
  }
  ...
  if (type === "Orden") {
    const { data: customer } = await supabase.from("customers").select("id").ilike("full_name", name).limit(1).maybeSingle();
    const orderNumber = `OT-${String(Date.now()).slice(-6)}`;
    const { data, error } = await supabase.from("work_orders").insert({
      order_number: orderNumber,
      customer_id: customer?.id ?? null,
      equipment: secondary || "Equipo por identificar",
      issue: detail || "Pendiente de descripción",
      status: "new",
      source: "presencial",
    }).select().single();
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
  ...
  if (type === "Cotización") {
    ...
    const { data, error } = await supabase.from("quotes").insert({
      quote_number: `CT-${String(Date.now()).slice(-6)}`,
      customer_id: customer?.id ?? null,
      status: "draft",
      subtotal: total,
      total,
