...
export async function GET() {
  ...
  const [customers, outProducts, appointments, orders, quotes, sales, technicians, conversations, reminders, inventorySummary] = await Promise.all([
    supabase.from("customers").select("id,full_name,phone,email,address,sector,province,municipality,address_reference_1,address_reference_2,latitude,longitude,location_precision,source,created_at,updated_at").eq("organization_id", org).order("created_at", { ascending: false }).limit(500),
    ...
    supabase.from("work_orders").select("id,order_number,order_type,service_category,service_line,warranty_type,distance_km,imei,operator_ticket,intake_channel,origin_branch,current_branch,customer_id,appointment_id,technician_id,equipment,brand,model,issue,status,priority,source,visit_fee,visit_fee_creditable,visit_fee_applied_to_invoice,customer_repair_approved,customer_repair_approved_at,quote_id,created_at,updated_at").eq("organization_id", org).order("created_at", { ascending: false }).limit(500),
  ...
  if (firstErrors.length) {
    return NextResponse.json({ ok: false, errors: firstErrors }, { status: 500 });
  }
