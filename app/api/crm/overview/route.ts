...
export async function GET() {
  ...
  const [customers, products, appointments, orders, quotes, sales, technicians, callEvents] = await Promise.all([
    supabase.from("customers").select("id,full_name,phone,email,address,sector,source,created_at").order("created_at", { ascending: false }).limit(250),
    ...
    supabase.from("work_orders").select("id,order_number,customer_id,appointment_id,technician_id,equipment,brand,model,issue,status,priority,source,visit_fee,visit_fee_creditable,visit_fee_applied_to_invoice,customer_repair_approved,customer_repair_approved_at,quote_id,created_at").order("created_at", { ascending: false }).limit(250),
