...
export async function POST(request: Request) {
  ...
  if (!body.id) {
    return NextResponse.json({ ok: false, error: "El producto es requerido." }, { status: 400 });
  }

  const name = text(body.name);
  if (!name) {
    return NextResponse.json({ ok: false, error: "El nombre del producto es requerido." }, { status: 400 });
  }
  ...
  if (error) {
    ...
    return NextResponse.json(
      { ok: false, error: duplicateSku ? "Ese SKU ya está asignado a otro producto." : error.message },
      { status: 500 },
    );
  }
