...
export async function POST(request: Request) {
  ...
  if (!body.customer_id || !body.product_id) {
    return NextResponse.json({ ok: false, error: "Cliente y producto son requeridos." }, { status: 400 });
  }
  ...
  const [{ data: customer, error: customerError }, { data: product, error: productError }] = await Promise.all([
    ...
    supabase
      .from("products")
      .select("id,name,piece_name,description,item_type,category,sale_price,price,max_discount_pct,minimum_authorized_price,installation_price,delivery_price,installation_includes_delivery,stock,reserved_stock")
      .eq("id", body.product_id)
      .single(),
  ]);

  if (customerError || productError || !customer || !product) {
    return NextResponse.json({
      ok: false,
      error: customerError?.message || productError?.message || "Datos no encontrados.",
    }, { status: 404 });
  }
  ...
  if (available < quantity) {
    return NextResponse.json({ ok: false, error: "No hay inventario suficiente para la cantidad solicitada." }, { status: 400 });
  }
  ...
  const largeEquipment = isLargeInstallableEquipment(product as unknown as Record<string, unknown>);
  const smallItem = isSmallDeliveryItem(product as unknown as Record<string, unknown>);
  const configuredInstallation = Number(product.installation_price || 0);
  const configuredDelivery = Number(product.delivery_price || 0);

  const installationAmount = body.include_installation
    ? largeEquipment
      ? configuredInstallation > 0 ? configuredInstallation : LARGE_EQUIPMENT_INSTALLATION_PRICE
      : configuredInstallation
    : 0;

  const deliveryFree = Boolean(body.include_installation && largeEquipment);
  const deliveryAmount = body.include_delivery && !deliveryFree
    ? smallItem
      ? configuredDelivery > 0 ? configuredDelivery : SMALL_ITEM_DELIVERY_PRICE
      : configuredDelivery
    : 0;
  ...
  const message = [
    ...
    deliveryFree ? "Envío: incluido con la instalación" : body.include_delivery ? `Envío: RD$${deliveryAmount.toLocaleString("es-DO")}` : "Sin envío",
  ...
  return NextResponse.json({
    ...
    available,
    pricing_policy: {
      installation: largeEquipment && body.include_installation ? LARGE_EQUIPMENT_INSTALLATION_PRICE : null,
      small_item_delivery: smallItem && body.include_delivery ? SMALL_ITEM_DELIVERY_PRICE : null,
    },
  });
}
