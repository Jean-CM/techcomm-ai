...
// "esperando_aprobacion" that haven't been called yet, and places an
// outbound call presenting the out-of-warranty repair quote for approval —
// this is the automation of the manual process tracked in the partner's
// real approval-call spreadsheet.
...
export async function GET(request: Request) {
  ...
  if (!apiKey || !agentId || !phoneNumberId) {
    return NextResponse.json({ ok: false, error: "Falta ELEVENLABS_API_KEY, ELEVENLABS_APPROVAL_AGENT_ID o ELEVENLABS_PHONE_NUMBER_ID" }, { status: 503 });
  }

  const admin = getSupabaseAdmin();
  const { data: due, error } = await admin
    .from("work_orders")
    .select("id, order_number, customer_id, brand, model, issue, quote_id")
  ...
  for (const order of due) {
    const [{ data: customer }, { data: quote }] = await Promise.all([
      order.customer_id ? admin.from("customers").select("full_name,phone").eq("id", order.customer_id).maybeSingle() : Promise.resolve({ data: null }),
      order.quote_id ? admin.from("quotes").select("total").eq("id", order.quote_id).maybeSingle() : Promise.resolve({ data: null }),
    ]);

    await admin.from("work_orders").update({ approval_call_attempted_at: new Date().toISOString() }).eq("id", order.id);
    ...
    try {
      ...
      results.push({ order_id: order.id, ok: callResponse.ok, conversation_id: payload?.conversation_id });
    } catch (err) {
      results.push({ order_id: order.id, ok: false, reason: err instanceof Error ? err.message : "unknown" });
    }
  }
