...
const ROLE_META: Record<Role,{label:string;description:string;menus:string[];permissions:Permission[]}> = {
  super_admin:{ label:"Super Admin", description:"Control total del CRM, operación y configuración.", menus:["Dashboard","Conversaciones","Clientes","Agenda","Técnicos","Órdenes","Ventas","Productos","Cotizaciones"], permissions:["edit_customer","edit_technician","edit_product","reschedule","reassign","update_order","manual_management","view_financial"] },
  admin:{ label:"Administrador", description:"Administración operativa completa sin configuración sensible.", menus:["Dashboard","Conversaciones","Clientes","Agenda","Técnicos","Órdenes","Ventas","Productos","Cotizaciones"], permissions:["edit_customer","edit_technician","edit_product","reschedule","reassign","update_order","manual_management","view_financial"] },
  secretary:{ label:"Secretaría", description:"Atención de clientes, agenda, reprogramaciones y gestiones presenciales.", menus:["Dashboard","Conversaciones","Clientes","Agenda","Órdenes","Cotizaciones"], permissions:["edit_customer","reschedule","manual_management"] },
  supervisor:{ label:"Supervisor técnico", description:"Citas, técnicos, órdenes, pendientes y reasignaciones.", menus:["Dashboard","Conversaciones","Clientes","Agenda","Técnicos","Órdenes"], permissions:["edit_technician","reschedule","reassign","update_order"] },
};
...
export default function OperationsClient(){
  ...
  const orders=useMemo(()=>data.work_orders.filter(item=>{
    const customer=customersById.get(item.customer_id||"");
    const tech=techniciansById.get(item.technician_id||"");
    const haystack=[item.order_number,customer?.full_name,item.equipment,item.brand,item.model,item.issue,tech?.full_name].filter(Boolean).join(" ").toLowerCase();
    return (!search||haystack.includes(search.toLowerCase()))&&(statusFilter==="all"||item.status===statusFilter)&&(secondaryFilter==="all"||String(item.technician_id||"unassigned")===secondaryFilter);
  }),[data.work_orders,customersById,techniciansById,search,statusFilter,secondaryFilter]);
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.content}>
      ...
      {!loading&&active==="Agenda"&&<section className={`${styles.card} ${styles.compactAgenda}`}>
        <div className={styles.cardHead}><div><span className={styles.eyebrow}>AGENDA COMPACTA</span><h3>Visitas programadas</h3><p className={styles.summary}>Vista reducida para interpretar horarios, clientes, técnicos y pendientes sin desplazamiento excesivo.</p></div><span className={styles.badge}>{appointments.length} citas</span></div>
        <div className={styles.filters}><input className={styles.input} value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar cliente, teléfono, dirección o servicio..."/><input className={styles.select} type="date" value={dateFilter} onChange={e=>setDateFilter(e.target.value)}/><select className={styles.select} value={statusFilter} onChange={e=>setStatusFilter(e.target.value)}><option value="all">Todos los estados</option>{["scheduled","confirmed","rescheduled","completed","cancelled"].map(value=><option key={value} value={value}>{statusLabel(value)}</option>)}</select><select className={styles.select} value={secondaryFilter} onChange={e=>setSecondaryFilter(e.target.value)}><option value="all">Todos los técnicos</option><option value="unassigned">Sin técnico</option>{data.technicians.map(item=><option key={item.id} value={item.id}>{item.full_name}</option>)}</select></div>
        <div className={styles.tableWrap}><div className={styles.table}><div className={`${styles.headRow} ${styles.colsAgenda}`}><span>Fecha y hora</span><span>Cliente</span><span>Servicio / dirección</span><span>Técnico</span><span>Estado</span><span>Acciones</span></div>{appointments.map(item=>{const customer=customersById.get(item.customer_id||"");return <div className={`${styles.row} ${styles.colsAgenda}`} key={item.id}><strong>{localDate(item.starts_at)}</strong><div><strong>{customer?.full_name||"Sin nombre"}</strong><p>{customer?.phone||"Sin teléfono"}</p></div><div><span>{item.notes||"Servicio"}</span><p>{item.address||customer?.address||"Sin dirección"}</p></div><span>{techniciansById.get(item.technician_id||"")?.full_name||"Sin técnico"}</span><span className={badgeClass(item.status)}>{statusLabel(item.status)}</span><div className={styles.actions}>{can("reschedule")&&actionButton("Gestionar",()=>setModal({kind:"appointment",item}))}</div></div>})}</div></div>
      </section>}
      ...
      {!loading&&active==="Órdenes"&&<section className={styles.card}>
        <div className={styles.cardHead}><div><span className={styles.eyebrow}>OPERACIÓN TÉCNICA</span><h3>Órdenes y pendientes</h3></div><span className={styles.badge}>{orders.length} órdenes</span></div>
        <div className={styles.filters}><input className={styles.input} value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar orden, cliente, equipo o falla..."/><select className={styles.select} value={statusFilter} onChange={e=>setStatusFilter(e.target.value)}><option value="all">Todos los estados</option>{["new","scheduled","assigned","in_progress","pending_customer","approved","on_hold","completed","cancelled"].map(value=><option key={value} value={value}>{statusLabel(value)}</option>)}</select><select className={styles.select} value={secondaryFilter} onChange={e=>setSecondaryFilter(e.target.value)}><option value="all">Todos los técnicos</option><option value="unassigned">Sin técnico</option>{data.technicians.map(item=><option key={item.id} value={item.id}>{item.full_name}</option>)}</select></div>
        <div className={styles.tableWrap}><div className={styles.table}><div className={`${styles.headRow} ${styles.colsOrder}`}><span>Orden</span><span>Cliente</span><span>Equipo</span><span>Falla</span><span>Técnico</span><span>Estado</span><span>Acción</span></div>{orders.map(item=><div className={`${styles.row} ${styles.colsOrder}`} key={item.id}><strong>{item.order_number}</strong><span>{customersById.get(item.customer_id||"")?.full_name||"Sin nombre"}</span><div><span>{item.equipment||"Equipo"}</span><p>{[item.brand,item.model].filter(Boolean).join(" · ")}</p></div><span>{item.issue||"Sin detalle"}</span><span>{techniciansById.get(item.technician_id||"")?.full_name||"Sin técnico"}</span><span className={badgeClass(item.status)}>{statusLabel(item.status)}</span><div>{can("update_order")&&actionButton("Gestionar",()=>setModal({kind:"order",item}))}</div></div>)}</div></div>
      </section>}

      {!loading&&active==="Productos"&&<section className={styles.card}>
