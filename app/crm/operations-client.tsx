    ...
    <section className={styles.content}>
      ...
      {!loading&&active==="Dashboard"&&<>
        ...
        <div className={styles.gridTwo}>
          <section className={styles.card}><div className={styles.cardHead}><div><span className={styles.eyebrow}>PRÓXIMAS VISITAS</span><h3>Agenda compacta</h3></div><button className={styles.secondary} onClick={()=>setActive("Agenda y Órdenes")}>Abrir</button></div>{data.appointments.slice(0,5).map(item=><div className={styles.row} key={item.id} style={{gridTemplateColumns:"145px 1fr 140px 105px"}}><strong>{localDate(item.starts_at)}</strong><div>{customersById.get(item.customer_id||"")?.full_name||"Sin nombre"}<p>{item.notes||"Servicio"}</p></div><span>{techniciansById.get(item.technician_id||"")?.full_name||"Sin técnico"}</span><span className={badgeClass(item.status)}>{statusLabel(item.status)}</span></div>)}</section>
