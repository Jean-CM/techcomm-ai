...
export default function CrmPage() {
  ...
  async function importCatalog(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (file.name.toLowerCase().endsWith(".csv")) {
      ...
      setMessage(`Importación completada: ${payload.imported} registros.`); await load(); return;
    }
    const form = new FormData(); form.append("file", file);
    const response = await fetch("/api/crm/import-file", { method: "POST", body: form });
    const payload = await response.json();
    if (!response.ok) { setMessage(payload.error ?? "No fue posible importar el archivo."); return; }
    setMessage(`Importación completada: ${payload.imported} registros.`); await load();
  }
  ...
  return <main className={styles.shell}>
    <aside className={styles.sidebar}>
      <div className={styles.identity}>
        <img src="https://www.techcommwireless.com/images/logo-2.png" alt="Techcomm Wireless" onError={(event) => { event.currentTarget.style.display = "none"; }} />
        <div><h2>Techcomm <span>Operations</span></h2><p>Operación omnicanal</p></div>
      </div>
    ...
    <section className={styles.content}>
      ...
      {!loading && active === "Dashboard" && <>
        <div className={styles.metrics}>
          <article><small>Conversaciones</small><strong>{data.call_events.length}</strong><span>Registros procesados</span></article>
      ...
      {!loading && active === "Conversaciones" && <section className={styles.card}><div className={styles.cardTitle}><div><small>HISTORIAL</small><h3>Llamadas y conversaciones procesadas</h3></div></div>{data.call_events.length ? data.call_events.map((item) => { const phone = (item.customer_phone || "").replace(/\D/g, ""); const customer = customersByPhone.get(phone); return <div className={styles.conversation} key={item.id}><div className={styles.channel}>☎</div><div><strong>{customer?.full_name || item.customer_phone || "Contacto sin identificar"}</strong><p>{item.summary || "Sin resumen disponible"}</p></div><div><span>{statusLabel(item.status)}</span><small>{new Date(item.created_at).toLocaleString("es-DO")}</small></div></div>; }) : empty("No hay conversaciones registradas.")}</section>}
